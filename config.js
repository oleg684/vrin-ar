// Конфигурация AR-выставки
const EXHIBITION_CONFIG = {
  artworks: [
    {
      id: "scene-mpo3rg4w",
      title: "Сцена 1",
      target: "assets/targets/scene-mpo3rg4w.jpg",
      model: "assets/models/scene-mpo3rg4w.glb",
      position: { x: -0.37, y: 0, z: 0 },
      scale: 0.16,
      rotation: { x: 0, y: -94.5, z: 0 }
    },
    {
      id: "scene-mppbsee9",
      title: "Сцена 3",
      target: "assets/targets/scene-mppbsee9.jpg",
      model: "assets/models/scene-mppbsee9.glb",
      position: { x: 0, y: -0.36, z: 0 },
      scale: 0.09,
      rotation: { x: 0, y: -101.1, z: 0 }
    },
    {
      id: "scene-mppbsuj9",
      title: "Сцена 4",
      target: "assets/targets/scene-mppbsuj9.jpg",
      model: "assets/models/scene-mppbsuj9.glb",
      position: { x: -0.1, y: -0.05, z: -2 },
      scale: 0.06,
      rotation: { x: 0, y: -182.9, z: 0 }
    },
    {
      id: "scene-mppc6f2z",
      title: "Сцена 5",
      target: "assets/targets/scene-mppc6f2z.jpg",
      model: "assets/models/scene-mppc6f2z.glb",
      position: { x: -0.02, y: -0.75, z: 0 },
      scale: 0.23,
      rotation: { x: 0, y: -215.9, z: 0 }
    },
    {
      id: "scene-mppc6qqk",
      title: "Сцена 6",
      target: "assets/targets/scene-mppc6qqk.jpg",
      model: "assets/models/scene-mppc6qqk.glb",
      position: { x: 0.28, y: 0.22, z: 0 },
      scale: 0.06,
      rotation: { x: 0, y: -100.6, z: 0 }
    },
    {
      id: "scene-mppc6rze",
      title: "Сцена 7",
      target: "assets/targets/scene-mppc6rze.jpg",
      model: "assets/models/scene-mppc6rze.glb",
      position: { x: 0, y: 0, z: 0 },
      scale: 0.06,
      rotation: { x: 0, y: -92.4, z: 0 }
    },
    {
      id: "scene-mppc6uwc",
      title: "Сцена 8",
      target: "assets/targets/scene-mppc6uwc.jpg",
      model: "assets/models/scene-mppc6uwc.glb",
      position: { x: 0, y: 0, z: 0 },
      scale: 0.06,
      rotation: { x: 0, y: 0, z: 0 }
    },
    {
      id: "scene-mppcccfs",
      title: "Сцена 9",
      target: "assets/targets/scene-mppcccfs.jpg",
      model: "assets/models/scene-mppcccfs.glb",
      position: { x: 0, y: 0, z: 0 },
      scale: 0.06,
      rotation: { x: 0, y: 0, z: 0 }
    },
    {
      id: "scene-mpqv3rfu",
      title: "Сцена 9",
      target: "assets/targets/scene-mpqv3rfu.jpg",
      model: "",
      position: { x: 0, y: 0, z: 0 },
      scale: 0.06,
      rotation: { x: 0, y: 0, z: 0 }
    }
  ]
};